import { hypotenuse } from '../utils/utils';
...
export class SingularValueDecomposition {
    ...
    constructor(value: Matrix) {
        ...
        const computeLeftSingularVectors = true;
        const computeRightSingularVectors = true;
        const autoTranspose = false;
    ...
    inverse(): Matrix {
        ...
        let vrows = V.rows;
        let vcols = V.columns;
        let X = Matrix.zeros(vrows, this.s.length);
        ...
        let urows = U.rows;
        let ucols = U.columns;
        let Y = Matrix.zeros(vrows, urows);
