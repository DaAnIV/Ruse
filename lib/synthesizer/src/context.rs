...
pub(crate) type ValuesHashMap = BTreeMap<VariableName, u64>;
...
pub struct ContextArray {
    pub(crate) depth: u32,
    pub(crate) inner: Vec<Box<Context>>,
}
...
impl ContextArray {
    ...
    pub fn subset(&self, other: &ContextArray) -> ContextSubsetResult {
        if self
            ...
            .any(|(self_ctx, other_ctx)| {
                self_ctx.subset(other_ctx) == ContextSubsetResult::NotSubset
            })
        {
            return ContextSubsetResult::NotSubset;
        }
